...
: ${CHARTMUSEUM_INSTALL_DIR:="/usr/local/bin"}
...
# checkChartmuseumInstalledVersion checks which version of chartmuseum is installed and
...
checkChartmuseumInstalledVersion() {
  if [[ -f "${CHARTMUSEUM_INSTALL_DIR}/${BINARY_NAME}" ]]; then
    ...
    if [[ "$version" == "$TAG" ]]; then
      ...
      echo "ChartMuseum ${TAG} is available. Changing from version ${version}."
      return 1
    fi
  else
    return 1
...
# for that binary.
downloadFile() {
  CHARTMUSEUM_DIST="chartmuseum-$TAG-$OS-$ARCH.tar.gz"
  DOWNLOAD_URL="https://get.helm.sh/$CHARTMUSEUM_DIST"
  CHECKSUM_URL="$DOWNLOAD_URL.sha256sum"
  CHARTMUSEUM_TMP_ROOT="$(mktemp -dt chartmuseum-installer-XXXXXX)"
  CHARTMUSEUM_TMP_FILE="$CHARTMUSEUM_TMP_ROOT/$CHARTMUSEUM_DIST"
  CHARTMUSEUM_SUM_FILE="$CHARTMUSEUM_TMP_ROOT/$CHARTMUSEUM_DIST.sha256sum"
...
# installFile installs the ChartMuseum binary.
installFile() {
...
verifySignatures() {
  ...
  gpg --batch --quiet --homedir="${gpg_homedir}" --import "${CHARTMUSEUM_TMP_ROOT}/${keys_filename}" 2> "${gpg_stderr_device}"
...
fail_trap() {
  ...
  if [ "$result" != "0" ]; then
    ...
    echo -e "\tFor support, go to https://github.com/helm/chartmuseum."
  fi
  cleanup
  exit $result
}
...
testVersion() {
  set +e
  CHARTMUSEUM="$(command -v $BINARY_NAME)"
  if [ "$?" = "1" ]; then
    echo "$BINARY_NAME not found. Is $CHARTMUSEUM_INSTALL_DIR on your "'$PATH?'
    exit 1
  fi
  set -e
}
...
cleanup() {
  if [[ -d "${CHARTMUSEUM_TMP_ROOT:-}" ]]; then
    rm -rf "$CHARTMUSEUM_TMP_ROOT"
  fi
}
...
verifySupported
checkDesiredVersion
if ! checkChartmuseumInstalledVersion; then
  downloadFile
  verifyFile
  installFile
fi
